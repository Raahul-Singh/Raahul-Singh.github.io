...
${content}
    ...
    <script src="js/main.js" defer></script>
      ...
      // For the index.html file, we handle both books and essays content together
      // We'll read the existing file to get any content already populated
      let recentBooks = '<!-- Loading recent books... -->';
      let recentEssays = '<!-- Loading recent essays... -->';
      
      // If selector is 'recent-books', update that content
      if (selector === 'recent-books') {
        recentBooks = content;
        // If the file exists, try to read existing essays content
        if (fs.existsSync(filePath)) {
          try {
            const existingContent = fs.readFileSync(filePath, 'utf8');
            const essaysMatch = existingContent.match(/<div id="recent-essays" class="featured-grid">\s*([\s\S]*?)\s*<\/div>/);
            if (essaysMatch && essaysMatch[1] && !essaysMatch[1].includes('Loading recent essays')) {
              recentEssays = essaysMatch[1];
            }
          } catch (err) {
            console.log('Could not read existing essays content, using default');
          }
        }
      }
      
      // If selector is 'recent-essays', update that content
      if (selector === 'recent-essays') {
        recentEssays = content;
        // If the file exists, try to read existing books content
        if (fs.existsSync(filePath)) {
          try {
            const existingContent = fs.readFileSync(filePath, 'utf8');
            const booksMatch = existingContent.match(/<div id="recent-books" class="featured-grid">\s*([\s\S]*?)\s*<\/div>/);
            if (booksMatch && booksMatch[1] && !booksMatch[1].includes('Loading recent books')) {
              recentBooks = booksMatch[1];
            }
          } catch (err) {
            console.log('Could not read existing books content, using default');
          }
        }
      }
...
<body>
    ...
    <main>
        <section class="hero">
            <div class="profile-container">
                ...
                <div class="profile-info">
                    <h1 class="hero-title">Hello, I'm Raahul</h1>
                    <p class="hero-subtitle">I write about technology, books, and ideas that matter.</p>
                    <div class="hero-buttons">
                        <a href="essays.html" class="btn btn-primary">Read My Essays</a>
                        <a href="books.html" class="btn btn-secondary">Book Reviews</a>
                    </div>
                </div>
            </div>
        </section>
        ...
        <section class="about-summary">
            <div class="container">
                <p>Welcome to my digital garden where I share thoughts on <span class="highlight">machine learning</span>, 
                <span class="highlight">software engineering</span>, and <span class="highlight">books</span> 
                that have influenced my thinking. My writing focuses on distilling complex ideas into practical insights.</p>
            </div>
        </section>
        ...
        <section class="featured-section">
            ...
            <div id="recent-books" class="featured-grid">
${recentBooks}
            </div>
        </section>
        ...
        <section class="featured-section">
            ...
            <div id="recent-essays" class="featured-grid">
${recentEssays}
            </div>
        </section>
        ...
        <section class="newsletter">
            <div class="container">
                <h2>Stay Updated</h2>
                <p>Subscribe to get notified when I publish new essays and book reviews.</p>
                <form class="subscribe-form" action="#" method="post">
                    <input type="email" placeholder="Your email address" required>
                    <button type="submit" class="btn btn-primary">Subscribe</button>
                </form>
                <p class="form-note">I respect your privacy and won't send spam.</p>
            </div>
        </section>
    </main>
...
async function compileStatic() {
  ...
  try {
          ...
          <div class="card-content">
            ${cover_image ? 
              `<div class="card-image">
                <a href="books/${id}.html" class="card-image-link">
                  <img src="${cover_image}" alt="Cover of ${title}" loading="lazy">
                </a>
              </div>` : ''}
            <div class="card-body">
              <h3 class="card-title"><a href="books/${id}.html">${title}</a></h3>
              <p class="card-meta">by ${author}</p>
              ${rating ? `<div class="rating">${'★'.repeat(Math.floor(parseFloat(rating)))}${parseFloat(rating) % 1 === 0.5 ? '½' : ''}</div>` : ''}
    ...
    const recentEssaysHtml = recentEssays.map(essay => {
      ...
      return `
        <div class="feature-card essay-card">
          <div class="card-content">
            <div class="card-body">
              <h3 class="card-title"><a href="essays/${id}.html">${title}</a></h3>
              <p class="card-date">${formatDate(date)}</p>
              ${tags && tags.length > 0 ? 
                `<div class="tags">${tags.map(tag => `<span class="tag">${tag}</span>`).join('')}</div>` : ''}
            </div>
    ...
    // Create clean index file first to ensure proper structure
    createCleanHtmlFile(path.join(websiteDir, 'index.html'), '', '');
    ...
    const updates = [
      ...
      // Now update the index page with recent content - do this separately after creating clean file
